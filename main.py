...
def create_diagram():
    ...
    # Display nodes with their numbers
    print("\nNodes list:")
    for num, name in nodes.items():
        print(f"{num}: {name}")
    
    print("\nNow define comments for nodes or connections between nodes.")
    while True:
        command = input("Enter a comment command (or type 'done' to proceed): ").strip()
